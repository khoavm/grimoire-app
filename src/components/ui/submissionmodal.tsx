...
const SubmissionModal: React.FC<SubmissionModalProps> = ({ quest, isOpen, onClose }) => {
                ...
                <div className="p-6 overflow-y-auto">
                    {isPending && (
                        <div className="absolute inset-0 z-10 bg-white/90 flex flex-col items-center justify-center animate-fadeIn">
                            ...
                            <h4 className="text-lg font-semibold text-slate-700">Đang chấm điểm...</h4>
                            <p className="text-sm text-slate-500">Hệ thống đang phân tích câu trả lời của bạn</p>
                        </div>
                    )}
                    ...
                    {isSuccess && result && (
                        <div className="space-y-4 text-center animate-fadeIn">
                            {/* Score Circle */}
                            <div className={`mx-auto w-24 h-24 flex items-center justify-center rounded-full border-4 text-3xl font-bold 
                                ${result.isCorrect ? 'border-green-500 text-green-600 bg-green-50' : 'border-orange-500 text-orange-600 bg-orange-50'}`}>
                                {result.score}
                            </div>

                            <div>
                                <h4 className={`text-xl font-bold ${result.isCorrect ? 'text-green-700' : 'text-orange-700'}`}>
                            ...
                            {result.improvementHint && (
                                <div className="bg-blue-50 p-4 rounded-lg border border-blue-100 text-left">
                                    <span className="text-xs font-bold text-blue-600 uppercase">Gợi ý cải thiện</span>
                                    <p className="text-sm text-blue-800 mt-1">{result.improvementHint}</p>
                                </div>
                            )}
                        </div>
                    )}
